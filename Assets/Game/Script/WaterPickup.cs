...
public class WaterPickup : MonoBehaviour
{
    ...
    [Tooltip("If ON, drinking too many water pickups damages the player.")]
    public bool useConsumptionLimit = true;

    [Tooltip("How many water pickups can be safely consumed before penalty starts.")]
    public int maxSafeConsumptions = 3;

    [Tooltip("Damage taken every time water is consumed after Max Safe Consumptions.")]
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        bool isOverConsumed =
            useConsumptionLimit &&
            totalWaterConsumed > maxSafeConsumptions;

        if (logConsumption)
        {
            Debug.Log(
                $"[WaterPickup] Water consumed: {totalWaterConsumed}. Over-consumed: {isOverConsumed}",
                this
            );
        }
        ...
        {
            // IMPORTANT:
            // Over-consumed water does NOT heal, does NOT reduce anxiety,
        ...
        {
            // Normal safe water pickup behavior.
            if (healthAmount > 0)
